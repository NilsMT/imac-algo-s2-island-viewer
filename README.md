...
- [Rapport](#rapport)
    ...
    - [Paramètres et impact visuel](#paramètres-et-impact-visuel)
        ...
        - [Aléatoire](#aléatoire)
        - [Intervale de spawn](#intervale-de-spawn)
    - [Difficultés et solutions](#difficultés-et-solutions)
        - [Le Simplex qui marchais pas](#le-simplex-qui-marchais-pas)
...
avec une interpolation linéaire (qui peut se désactiver) si la valeur de correspond pas parfaitement à un pixel
...
Cela a été décidé car nous avions plusieurs bruit à disposition alors pourquoi pas en faire une fonctionnalité pour à la fois la pile de bruit et le masque.
...
(PS: le fetch-content dans le CMake ne marchais pas sur ce repo pour une raison obscure)
...
Il a été décidé d'ajouter des paramètres sur le placement aléatoire des objets pour faire varié la taille et l'orientation pour avoir de la variation sur les objets, et avoir un rendu plus proche de la vie réel (çàd les arbres ne sont pas tous de la mêmes tailles)

### Intervale de spawn

Une intervale de hauteur de spawn autorisé a été codé pour permettre une condition de placement pour, par exemple, évité d'avoir des objets trop haut dans les montagnes, ou dans la mer.

## Difficultés et solutions

### Le Simplex qui marchais pas
...
Après avoir crié à l'aide au chrgé de TD, qui n'a pas trouvé la solution car le code était une exacte copie de ce que l'article avait,
...
à un moment du projet, le placement des objets pouvait être aléatoire. Cependant ces valeurs aléatoires changeait à chaque frame car la fonction d'aléatoire `randF` était appelé pendant le rendu.

![](./img/object_dubstep.gif)

#### La solution

À la construction de la carte et du placement des points, les données aléatoires de placement on été stocké dans une liste de `ObjectRandomizationData` disponible dans le `context` de l'application. Cette structure contient la modification de la position et la rotation de base de l'objet, pour chacun. Résultat : On récupère la donnée générée et stockée bien avant, ce qui ne la change pas à chaque frame.

### L'interface pas ergonomique

#### Le problème

Avant, il y avait uniquement une fenêtre avec toutes les morceaux d'interfaces listé dedans, un poil séparé les un des autres mais rien de vraiment visible. Et à mesure que le projet progressait il y avait beaucoup de choses.
D'autant plus qu'il y avait la pile de bruit à géré qui ajoutait beaucoups de lignes

![](./img/old_menu.png)

#### Solution

Il a donc été décidé de refaire l'interface sous formes de sous-menu avec des couleurs différentes et des sections propres à chaques catégories de paramètres
...
Avec plus de temps, en témoigne la liste de tâches, nous aurions pu implémenté un outil de création de color map plutôt que de chargé une image, et ce avec [imgui_gradient](https://github.com/Coollab-Art/imgui_gradient) de Coollab

Nous aurions pu faire comme dans minecraft et mettres des biomes selon un set d'objets 3D, une color map, et placés selon une carte de températures

Nous aurions pu ajouter plus de bruits, pour avoir des formes d'îles comment un croissant, un donut, des carrés etc...
...
La répartition a été faite de sorte à ce que les choses déjà manipulé par Nils pendant le workshop de prog algo (avec le Diamond Square) et le [Workshop d'esthétique et algorithmique](https://imac-wk-esthe-et-algo.vercel.app/src/free/island/index.html) soit faitent pas lui pour garantir une efficacité sur le reste du projet. Matthieu a été le plus en charge des fonctionnalités nécessaires du projet (çàd la base) pour garantir un code compréhensible.
